...
def db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS pass (
...
class SafeHavenE(QWidget):
    def __init__(self):
        ...
        self.attempts = 0
        self.max_attempts = 3
        self.timer_duration = 60 * 5  # 5 minutes
        self.timer = QTimer(self)  # Create a QTimer object
        self.timer.timeout.connect(self.update_timer)  # Connect the QTimer to the update_timer method
        db()
        self.check_previous_attempts()
    ...
    def start_timer(self):
        ...
        c.execute("INSERT INTO attempts (last_attempt_time) VALUES (?)", (last_attempt_time,))
        conn.commit()
        conn.close()
    ...
    def check_previous_attempts(self):
        conn = sqlite3.connect('db.db')
        c = conn.cursor()
        c.execute("SELECT last_attempt_time FROM attempts")
        row = c.fetchone()
        conn.close()

        if row is not None:
            last_attempt_time = row[0]
            elapsed_time = time.time() - last_attempt_time
            remaining_time = self.timer_duration - int(elapsed_time)
            if remaining_time > 0:
    ...
    def update_timer(self):
        ...
        if row is not None:
            ...
            if remaining_time > 0:
                ...
                self.timer_label.setText(f'Please wait {minutes:02d}:{seconds:02d} before the next try')
            else:
                self.reset_attempts()
